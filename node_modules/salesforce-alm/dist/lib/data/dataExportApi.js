...
class DataExportApi {
    ...
    _processRecords(options, parentRef, record, sobjectTree) {
        ...
        return this._processRecordAttributes(options, record, treeRecord, objRefId)
            .then(() => {
            ...
            return Promise.resolve(sobjectTree);
        });
    }
            ...
            if (key === 'attributes' || key === 'Id') {
                ...
                return this._loadMetadata(record.attributes.type)
                    ...
                    if (this._isQueryResult(metadata, key)) {
                        ...
                        return this._loadMetadata(field.records[0].attributes.type)
                            .then((childMetadata) => this._processRecordList(options, field, {
                            id: `@${objRefId}`,
    ...
    _loadMetadata(objectName) {
        ...
        return Promise.resolve(describe[objectName]);
    }
    ...
    _saveRecordRef(obj, refId) {
        ...
        if (this.referenceRegistry[obj.attributes.type][id]
    ...
    _generateDataPlan(options, sobjectTree) {
        ...
        const writeDataFileFn = (key) => () => this._writeObjectTypeDataFile(options, key, this.objectTypeRegistry[key].saveRefs, this.objectTypeRegistry[key].resolveRefs, `${key}s.json`, objects[key])
            .then((dataPlanPart) => {
        ...
        return srcDevUtil.sequentialExecute(writeDataFileFns)
            .then(() => this._writeFile(options, dataPlanFile, dataPlan));
    }
    ...
    _writeObjectTypeDataFile(options, type, saveRefs, resolveRefs, fileName, sObject) {
        ...
        return this._writeFile(options, fileName, sObject)
            .then(() => dataPlanPart);
    }
    /**
    ...
    _finalApplyRefs(sobjectTree) {
        ...
        return Promise.resolve({ records: sobjectTree });
    }
    _countRecords(obj) {
    ...
    _writeFile(options, fileName, jsonObject) {
        ...
        return fs.writeFileAsync(fileName, JSON.stringify(jsonObject, null, 4))
            .then(() => {
