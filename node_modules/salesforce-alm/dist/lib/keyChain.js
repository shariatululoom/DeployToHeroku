...
const messages = require(path.join(__dirname, 'messages'))(new ConfigApi().getLocale());
...
class KeychainAccess {
    ...
    validateProgram() {
        return _validateProgram(this.osImpl.getProgram(), this.fsIfc, _isExe);
    }
    /**
    ...
    setPassword(opts, fn) {
        ...
        _validateProgram(this.osImpl.getProgram(), this.fsIfc, _isExe);
...
const _windowsImpl = {
    ...
    setProgramOptions(opts) {
        return ['-ExecutionPolicy', 'ByPass', '-File', _windowsImpl.credManPath, '-AddCred',
        ...
        if (code !== 0) {
            switch (code) {
                ...
                default:
                    ...
                    err.action = messages.getMessage('keychainPasswordNotFoundAction', [`${_darwinImpl.getProgram()} ${_optionsToString(_darwinImpl.getProgramOptions(opts))}`]);
            }
            fn(err, null);
            return;
        }
...
class GenericKeychainAccess {
    ...
    isValidFileAccess(cb) {
        ...
        fs.mkdir(path.join(srcDevUtil.getGlobalHiddenFolder()), _constants.DEFAULT_USER_DIR_MODE, () => {
        ...
        super.isValidFileAccess((err, stats) => {
            if (!_.isNil(err)) {
                ...
                if (octalModeStr === EXPECTED_OCTAL_PERM_VALUE) {
                    ...
                    cb(almError('genericUnixKeychainInvalidPerms', null, 'genericUnixKeychainInvalidPermsAction', [this.getSecretFile(), EXPECTED_OCTAL_PERM_VALUE]));
                }
            }
        });
    }
...
module.exports[platforms.GENERIC_UNIX] = new GenericUnixKeychainAccess();
module.exports[platforms.GENERIC_WINDOWS] = new GenericWindowsKeychainAccess();
module.exports[platforms.WINDOWS] = new KeychainAccess(_windowsImpl, fs);
