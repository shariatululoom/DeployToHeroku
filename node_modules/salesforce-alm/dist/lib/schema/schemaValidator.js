...
const { readJSON } = require(path.join(__dirname, '..', 'srcDevUtil'));
const SCHEAMS_DIR = path.join(__dirname, '..', '..', '..', 'schemas');
/**
...
const loadExternalSchemas = (uri) => {
    const schemaPath = path.join(SCHEAMS_DIR, `${uri}.json`);
 ...
 */
const errorsText = (errors, schema) => errors.map(error => {
    const property = error.path.match(/^([a-zA-Z0-9\.]+)\.([a-zA-Z0-9]+)$/);
    const getPropValue = (prop) => error.path.split('.').reduce((obj, name) => (obj.properties && obj.properties[name]) || (name === '0' && obj.items) || obj[name] || obj[prop], schema);
    const getEnumValues = () => {
    ...
    validate(data) {
        return this.loadSchema()
            ...
            if (refs) {
                ...
                return Promise.all(refs.map(ref => loadExternalSchemas(ref)))
                    .then(externalSchemas => {
