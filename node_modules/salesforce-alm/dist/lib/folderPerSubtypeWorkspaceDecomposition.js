...
class FolderPerSubtypeWorkspaceDecomposition {
    ...
    getNondecomposedFilePath(decomposedPath, typeDef) {
        return path.join(this.getNondecomposedSourceDir(decomposedPath, typeDef), FolderPerSubtypeWorkspaceDecomposition.getNondecomposedFileName(decomposedPath, typeDef));
    }
    static getNondecomposedFullName(decomposedPath, typeDef) {
        return srcDevUtil.getNondecomposedFullName(decomposedPath, typeDef, MetadataRegistry.getMetadataFileExt());
    }
    static getNondecomposedFileName(decomposedPath, typeDef) {
        return `${FolderPerSubtypeWorkspaceDecomposition.getNondecomposedFullName(decomposedPath, typeDef)}.${typeDef.ext}${MetadataRegistry.getMetadataFileExt()}`;
    }
    getNondecomposedSourceDir(decomposedPath, typeDef) {
        let sourceDir = this.getDecomposedSourceDirFromDecomposedFile(decomposedPath, typeDef);
        if (!this.decompositionConfig.isGlobal) {
            sourceDir = path.dirname(sourceDir); // Account for entity-specific subdirectory
        }
        return sourceDir;
    }
    getDecomposedSourceDirFromDecomposedFile(decomposedPath, typeDef) {
        const isContainer = path.basename(decomposedPath).endsWith(`${typeDef.ext}${MetadataRegistry.getMetadataFileExt()}`);
        let sourceDir = path.dirname(decomposedPath);
        if (this.decompositionConfig.decompositions.length > 1 && !isContainer) {
            sourceDir = path.dirname(sourceDir); // Account for decomposition-specific subdirectory
        }
        return sourceDir;
    }
