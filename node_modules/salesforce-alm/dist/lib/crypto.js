...
const keyChainImpls = require(path.join(__dirname, 'keyChainImpl'));
...
const KeychainPromises = {
    ...
    migrate(oldKeychain, newKeychain, service, account) {
        return KeychainPromises.get(oldKeychain, service, account)
            .then((passwordResult) => KeychainPromises.set(newKeychain, service, account, passwordResult.password));
    }
};
/**
...
class Crypto {
    ...
    init(retryStatus, platform = os.platform()) {
        ...
        return this.keychainImpls.retrieveKeychainImpl(platform)
            .then((keychainImpl) => {
            logger.debug('keychain retrieved');
            return KeychainPromises.get(keychainImpl, KEY_NAME, ACCOUNT).then((savedKey) => {
                ...
                if (err.name === 'PasswordNotFound') {
                    ...
                    return KeychainPromises.set(keychainImpl, KEY_NAME, ACCOUNT, key)
            ...
            if (err.name === keyChainImpls.DEPRECATED_KEYCHAIN) {
                ...
                return KeychainPromises.migrate(err.fromKeychain, err.toKeychain, KEY_NAME, ACCOUNT)
                    .then(() => this.init('MIGRATED', platform));
            }
            throw err;
        });
    }
