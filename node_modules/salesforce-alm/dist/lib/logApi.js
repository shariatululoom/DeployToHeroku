...
class Mode {
    static get types() { return ['production', 'development']; }
    constructor(mode) {
...
class Logger extends bunyan {
    ...
    init(level, logFile = path.join(path.join(os.homedir(), '.sfdx', DEFAULT_LOG_FILE))) {
        ...
        if (this.useRingBuffer) {
            ...
            if (!fs.existsSync(logFile)) {
                mkdirp.sync(path.dirname(logFile), { mode: _constants.DEFAULT_USER_DIR_MODE });
                fs.writeFileSync(logFile, '', { mode: _constants.DEFAULT_USER_FILE_MODE });
            }
    ...
    getLogContentsAsText() {
        if (this.useRingBuffer) {
            return Promise.resolve(this.getBufferedRecords().reduce((accum, value) => {
            ...
            return Promise.reject(new Error('Log type is neither a file stream or ring buffer'));
        }
    }
    /**
    ...
    error(...args) {
        const consoleLog = args.length && args.length > 1 && _.isBoolean(args[0]) ?
            args[0] : true;
        if (consoleLog && this.humanConsumable) {
    ...
    get color() {
        ...
        colorFns['stripColor'] = heroku.color.stripColor;
        return colorFns;
    }
    /**
    ...
    /**
     * Gathers usage data about the CLI, command, error rate, and average exec time
     * and calls force.logServerDiagnostics.
     *
     * @param org The dev hub org
     * @param usages Array of usage data for CLI and commands
     * @returns {*|Promise.<T>}
     */
    logServerUsages(org, usages) {
        return org.getConfig()
            .then(() => org.force.logServerDiagnostics(org, usages))
            .catch((promiseError) => {
    ...
    logServerError(err, force, context) {
        ...
        const Org = require(path.join(__dirname, 'scratchOrgApi')); // eslint-disable-line global-require
        ...
        const orgToLogAgainst = orgApi || hubOrgApi;
        ...
        return (orgApi ? orgApi.getConfig() : Promise.resolve({}))
            .then((orgConfig) => ({
            commandName: cmdName,
            commandParams,
            ...
            // companyName: // TODO: where do we get this?
            ...
            .then(logData => hubOrgApi.getConfig()
            .then(hubConfig => Object.assign(logData, { hubOrgId: hubConfig.orgId })))
            .then(logData => force.logServerError(orgToLogAgainst, [logData]))
            .catch((promiseError) => {
            if (promiseError.name === 'InvalidProjectWorkspace' || promiseError.name === 'NoOrgFound') {
                return Promise.resolve();
            }
