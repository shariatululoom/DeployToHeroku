...
class OrgDescribeApi {
     ...
     * @returns {Promise}  a promise resolved with the org connection details
     */
    describeOrg(org) {
        if (_.isNil(org)) {
        ...
        return org.refreshAuth()
            ...
            if (isDevHub) {
                usernamePromise = Promise.resolve([`${org.username}.json`]);
            }
            else if (orgDesc.devHubId) {
                usernamePromise = Promise.resolve([`${orgDesc.devHubId}.json`, `${orgDesc.username}.json`]);
            }
            else {
            ...
            if (metaConfigsGroupedByOrgType) {
                orgInfo = metaConfigsGroupedByOrgType.nonScratchOrgs.get(org.getName()) ||
            ...
            if (clientId) {
                ...
                if (refreshToken) {
                    ...
                    instanceUrl = instanceUrl.startsWith('http') ?
                        instanceUrl.substring(instanceUrl.indexOf('//') + 2) : instanceUrl;
            ...
            return Promise.resolve(orgDesc);
        });
    }
}
