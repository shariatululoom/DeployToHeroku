...
const openBrowser = Promise.promisify(require('open'));
// Local
...
const _getUrl = function (context, orgApi) {
    return orgApi.force.getOrgFrontDoor(orgApi)
        .then((url) => {
        let newUrl = url;
...
const _checkLightningDomain = (myDomain, logger, retryCount = 0) => dns.lookupAsync(`${myDomain}.lightning.force.com`)
    .then(ip => {
    if (retryCount !== 0) {
    ...
    if (retryCount >= getDomainRetries()) {
        ...
        return Promise.delay(1000).then(() => _checkLightningDomain(myDomain, logger, ++retryCount));
    }
});
/**
...
const _open = function (context, orgApi) {
    return _getUrl(context, orgApi).then((url) => {
        const orgData = orgApi.getConfig().then(({ orgId, username }) => Promise.resolve({ url, orgId, username }));
        const act = () => context.urlonly ? orgData : openBrowser(url).then(() => orgData);
