...
const MdapiConvertApi = require(path.join(__dirname, 'mdapiConvertApi'));
...
class MdapiConvertCommand {
    ...
    execute() {
        return MetadataRegistry.initializeMetadataTypeInfos(this.org)
            ...
            .then((outputElements) => {
            srcDevUtil.logUnsupportedMimeTypeError(this.api.unsupportedMimeTypes, this.logger, this.force);
    ...
    getHumanSuccessMessage(data) {
        ...
        const columns = syncCommandHelper.getColumnMetaInfo(this.messages);
        const converted = [];
        const duplicates = [];
        data.forEach(e => {
