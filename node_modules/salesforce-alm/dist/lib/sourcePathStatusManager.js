...
const _sourceState = require(path.join(__dirname, 'sourceState'));
 ...
 * @param {string} workspacePath - The file system path to the workspace directory
 * @param forceIgnore - the parsed contents of .forceignore
 * @returns {boolean}
 * @private
 */
const _isValidSourcePath = function (sourcePath, workspacePath, forceIgnore) {
    let isValid = true;
    if (forceIgnore !== null) {
        isValid = forceIgnore.accepts(sourcePath.substr(workspacePath.length));
    }
    ...
    return !basename.startsWith('.') && !basename.endsWith('.dup') && isValid && !isPackage2ConfigFile;
};
/**
 ...
 * @param {MetadataRegistry} metadataRegistry - Optionally takes in the database of metadata types
 */
_SourcePathInfo.prototype.initFromPath = function (sourcePath, deferContentHash, metadataRegistry) {
    ...
    if (!util.isNullOrUndefined(metadataRegistry)) {
        if (!this.isDirectory) {
            if (!metadataRegistry.isValidSourceFilePath(this.sourcePath)) {
...
_SourcePathInfo.prototype.getPendingPathInfo = function () {
    ...
    if (pendingPathInfo.isDirectory ||
 ...
 * @private
 */
const _getNewPathInfos = function (sourcePath, workspacePath, forceIgnore, metadataRegistry) {
    let newPathInfos = [];
    if (_isValidSourcePath(sourcePath, workspacePath, forceIgnore)) {
        ...
        if (newPathInfo.isDirectory) {
            ...
            files.map((file) => path.join(sourcePath, file))
                .forEach((file) => {
                newPathInfos = newPathInfos.concat(_getNewPathInfos(file, workspacePath, forceIgnore, metadataRegistry));
            });
        }
    }
    return newPathInfos;
...
const _updateFromPath = function (sourcePath, workspacePathInfos, forceIgnore, isWorkspace, isArtifactRoot, metadataRegistry) {
    ...
    if (sourcePathInfo.isDirectory) {
        ...
        dirfiles.map((file) => path.join(sourcePath, file)).filter((file) => _isValidSourcePath(file, sourcePath, forceIgnore)).forEach((file) => _updateFromPath(file, workspacePathInfos, forceIgnore, false, false, metadataRegistry));
    }
};
/**
...
const _writeSourcePathInfos = function (org, workspacePath, workspacePathInfos) {
    ...
    if (util.isNullOrUndefined(workspacePathInfos.get(workspacePath))) {
 ...
 * @param {string} workspacePath - The path for the workspace
 * @param forceIgnore - The parsed contents of .forceignore
 * @param {MetadataRegistry} - The database of metadata types
 ...
 * @private
 */
const _processChangedDirectory = function (directoryPath, workspacePathInfos, workspacePath, forceIgnore, metadataRegistry) {
    ...
    files.map((file) => path.join(directoryPath, file)).filter((file) => 
    // We only need to process additions to the directory, any existing ones will get dealt with on their own
    !workspacePathInfos.has(file)).forEach((file) => {
        // This is new, so add it to the change list
        updatedPathInfos = updatedPathInfos.concat(_getNewPathInfos(file, workspacePath, forceIgnore, metadataRegistry));
    });
    return updatedPathInfos;
};
/**
 * Read the data model from the workspace
 * @param {string} org - The org the source file belong to
...
const _readSourcePathInfos = function (org) {
    ...
    try {
        ...
        workspacePathChanged = !util.isNullOrUndefined(oldWorkspacePath) && workspacePath !== oldWorkspacePath;
 ...
 * @constructor
 */
function SourcePathStatusManager(org) {
    this.org = org;
    this.workspacePath = org.config.getProjectPath();
    this.metadataRegistry = new MetadataRegistry(this.org);
    try {
        this.forceIgnore = parser.compile(fs.readFileSync(path.join(this.workspacePath, '.forceignore'), 'utf8'));
    }
    catch (err) {
        this.forceIgnore = null;
    }
    // Try to load the saved source state
    [this.workspacePathInfos, this.artifactRootPaths] = _readSourcePathInfos(this.org);
    if (util.isNullOrUndefined(this.workspacePathInfos)) {
...
/**
 * Get path infos for the source workspace
 * @param changesOnly - If true then return only the updated source paths (changed, new, or deleted)
 * @param packageDirectory - The package directory from which to fetch the sourcePathInfos
 * @returns {Array} - The path infos for the workspace
 */
SourcePathStatusManager.prototype.getSourcePathInfos = function (changesOnly, packageDirectory) {
    ...
    if (!util.isNullOrUndefined(packageDirectory) && util.isNullOrUndefined(currentArtifactRootPaths.find(rootDir => packageDirectory.startsWith(rootDir)))) {
    ...
    for (const sourcePathInfo of this.workspacePathInfos.values()) {
        let shouldIncludeSourcePathInfo = util.isNullOrUndefined(packageDirectory) || sourcePathInfo.sourcePath.includes(packageDirectory);
        if (this.forceIgnore !== null && this.forceIgnore.denies(sourcePathInfo.sourcePath.substr(this.workspacePath.length))) {
            shouldIncludeSourcePathInfo = false;
        }
        const pendingSourcePathInfo = sourcePathInfo.getPendingPathInfo();
        if (util.isNullOrUndefined(pendingSourcePathInfo)) {
            ...
            if (shouldIncludeSourcePathInfo) {
                ...
                if (pendingSourcePathInfo.isDirectory && !pendingSourcePathInfo.isDeleted() && !pendingSourcePathInfo.isWorkspace) {
                    // If it's a directory and it isn't deleted then process the directory change
                    sourcePathInfos = sourcePathInfos.concat(_processChangedDirectory(pendingSourcePathInfo.sourcePath, this.workspacePathInfos, this.workspacePath, this.forceIgnore, this.metadataRegistry));
                }
            }
        }
    }
